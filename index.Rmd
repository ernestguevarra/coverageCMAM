...
Here is a mathematical representation of the concept of coverage. The denominator implies that there is a criteria or a standard for eligibility to the programme.
...
Now we look at coverage specific to CMAM
...
$\Large \text{Treatment coverage} ~ = ~ \frac{C_{in} ~ + ~ C_{out}}{C_{in} ~ + ~ C_{out} ~ + ~ R_{in} ~ + ~ R_{out}}$
